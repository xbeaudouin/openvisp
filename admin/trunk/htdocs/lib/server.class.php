...
AND server_job.active = 1";
	...
	function fetch_server_role_list($role_name){
		...
		$query = "SELECT server_job.login, server_job.password, server_job.port, server_ip.public as ip_public, server_ip.private as ip_private, server_apps.apps, server_job.instance
FROM server_jobmodel, server_job, server_ip, server_apps
WHERE server_jobmodel.role='".$role_name."'
AND server_jobmodel.id=server_job.server_jobmodel_id
AND server_job.active = 1
