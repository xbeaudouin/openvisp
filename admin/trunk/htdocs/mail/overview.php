...
if ($_SERVER['REQUEST_METHOD'] == "GET")
{
   ...
   if ( $fDomain != NULL)
   {
    ...
    elCell.innerHTML = "<a href=" + oRecord.getData(column_name) + ">" + sData + "</a>"; 
		 ...
		 $function_name = "formatUrl";
		 $function_content2 = 'function(elCell, oRecord, oColumn, oData) { 
              var column_name = oColumn.key + "_url";
alert("NGO" + column_name);
	            elCell.innerHTML = "<a href=" + oRecord.getData(column_name) + ">" + oData + "</a>"; 
	        };';

		 $function_content = 'function(e){
        alert("NGO "+this.get("recordset")); }';

		 $function_content3 = 'function(e){
        var rs = this.get("recordset"),
            row = e.currentTarget,
            rec = rs.getRecord( row.get(\'id\') );
        
        
        this.highlightRow(row, lastrow);
        
        lastrow = row;
        lastrowIndex = rs.indexOf(rec);

        alert( "Record Index is : " + lastrowIndex );
}
